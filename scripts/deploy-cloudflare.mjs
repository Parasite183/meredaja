...
//   (functions/_middleware.js at the repo root) — Cloudflare Pages
...
//             server/seed-d1.sql generated (npm run deploy:seed)
//
...
// Find the worker's public URL (e.g. https://meredaja-api.<account>.workers.dev).
...
if (!workerUrl) {
  ...
  console.error('  `npx wrangler pages deploy web/dist --project-name meredaja` after');
